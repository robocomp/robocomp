...
/*	
	...
	void addPhysicMeshes( InnerModelNode* node ) {
		// If the node is a mesh, add it to the world
		InnerModelBody* body = dynamic_cast<InnerModelBody*>( node );
		if( body != NULL ) {
			printf( "Adding mesh %s\n", node->id.toLatin1().data() );
			const RTMat mat = innerModel->getTransformationMatrix( "world", node->id );
	...
	void startPhysics() {
		...
		// Create the floor
		btTransform floorTransform ( btQuaternion ( 0,0,0,1 ), btVector3 ( 0,0,0 ) );
		world->addStaticBody( floorShape, floorTransform, 0.6f );
		
		// Create a sphere
		float ballMass = 1.0f;
		btTransform ballTransform ( btQuaternion ( 0,0,0,1 ), btVector3 ( 0,0,5 ) );
		osg::ref_ptr<osg::MatrixTransform> ballOSG( new osg::MatrixTransform );
		ballOSG->addChild( viewer->addBall( osg::Vec3(0.0f, 0.0f, 0.0f), 0.2f, osg::Vec4(0.0f, 1.f, 0.f, 0.5f) ) );
		worker->getRootGroup()->addChild( ballOSG );
		world->addRigidBody( ballShape, new MotionOSG( ballTransform, ballOSG ), ballMass, 0.6f );
		
		// Convert the InnerModel tree to Bullet
		addPhysicMeshes( innerModel->getRoot() );
// 		QHash<QString, IMVMesh>::iterator it;
// 		int i = 1;
// 		for( it = imv->meshHash.begin() ; it != imv->meshHash.end() ; ++it ) {
