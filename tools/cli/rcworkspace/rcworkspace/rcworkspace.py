...
@app.command()
def add(directory: Optional[Path] = typer.Argument(None, help="Dir to start removing workspace"), accept_all: bool = False):
    """
    Add workspaces searching in the given directory
    """
    try:
        ws.add_workspace(directory, accept_all=accept_all)
    except KeyboardInterrupt:
        print("\nCanceled")
