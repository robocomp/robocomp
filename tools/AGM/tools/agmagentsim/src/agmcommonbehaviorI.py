...
class AGMCommonBehaviorI(AGMCommonBehavior):
	...
	def reloadConfigAgent(self, c):
		return self.worker.reloadConfigAgent()
	def activateAgent(self, prs, c):
		return self.worker.activateAgent(prs)
	def setAgentParameters(self, prs, c):
		return self.worker.setAgentParameters(prs)
	def getAgentParameters(self, c):
		return self.worker.getAgentParameters()
	def killAgent(self, c):
		return self.worker.killAgent()
	def uptimeAgent(self, c):
		return self.worker.uptimeAgent()
	def deactivateAgent(self, c):
		return self.worker.deactivateAgent()
	def getAgentState(self, c):
		return self.worker.getAgentState()
