...
class ComponentGroup:  ##On working condition
    def __init__(self):
        self.group_name = ""
        self.group_icon_file_path = ""
        self.group_icon_pixmap = None
        self.Components = []

    def __repr__(self):
        string = ""
        string = string + self.group_name
        string = string + self.group_icon_file_path
        return string

    def setName(self, name):
        self.group_name = name

    def set_icon_file_path(self, string):
        self.group_icon_file_path = string

    # This will read from the iconFilePath and set the pixMap "Dont Forget to call the Function after name of file is assigned"
    def read_from_icon_file(self):
        try:
            if self.group_icon_file_path == "":
                raise NameError("groupIconFilePath is Null")
            self.group_icon_pixmap = QtGui.QPixmap(self.group_icon_file_path)
        except Exception, e:
            raise e

    def add_component(self, component):  ##Add Component to the group
        ...
        component.groupName = self.group_name
        component.setGroup(self)
        component.graphicsItem.Icon = self.group_icon_pixmap
        component.DirectoryItem.setIcon(self.group_icon_pixmap)

    def remove_component(self, component):  # Remove the component from the group
        print "Removing compoent"
        self.Components.remove(component)
        component.group = None
        component.groupName = ""
        component.graphicsItem.Icon = QtGui.QPixmap(get_default_icon_path())
        component.DirectoryItem.setIcon(QtGui.QPixmap(get_default_icon_path()))

    def up_group_components(self, Logger):
        for x in self.Components:
            up_component(x, Logger)
...
class Logger:  # This will be used to log data
    def __init__(self, log_area, file=None):
        self.logArea = log_area
        self.file = file
...
class VisualNode(QtGui.QGraphicsItem):  ##Visual Node GraphicsItem
    ...
    def __init__(self, view=None, Alias="Component", parent=None):
        ...
        self.Icon = QtGui.QPixmap(get_default_icon_path())
    ...
    def paintMainShape(self,
                       painter):  ##This will draw the basic shape of a node.The big square and its containing elements
        pen = QtGui.QPen(
            QtGui.QColor.fromRgb(self.parent.nodeColor[0], self.parent.nodeColor[1], self.parent.nodeColor[2]))
        pen.setWidth(3)
        painter.setPen(pen)
        ...
        self.statusRect = QtCore.QRect(22, 10, 20,
                                       20)  ##The rectange shape on which the status of the node will be displayed
    ...
    def drawUpPort(self, painter,
                   brush):  ##The bulging arcs will can be calleds as the port because all connection starts from there
        painter.setBrush(brush)
...
def findWhichPorts(fromComponent,
                   toComponent):  # This will select out of the 4 ports which have to be connected to connection
...
class CompInfo(
    QtCore.QObject):  ##This contain the general Information about the Components which is read from the files and created
    def __init__(self, view=None, mainWindow=None, name="Component"):
        ...
        self.Ip = ""
        self.IconFilePath = get_default_icon_path()
        self.status = False
...
class DirectoryItem(QtGui.QPushButton):  # This will be listed on the right most side of the software
    def __init__(self, parent=None, args=None, name="Component"):
        ...
        QtGui.QPushButton.setIcon(self, QtGui.QIcon(QtGui.QPixmap(get_default_icon_path())))
        self.setText(name)
    ...
    def contextMenuEvent(self, event):
        self.parent.View.CompoPopUpMenu.set_component(self.parent.graphicsItem)
    ...
    def clickEvent(self):  # What happens when clicked
        ...
        if index == 0:
            self.parent.CommonProxy.set_visibility(True)
...
class commonBehaviorComponent(threading.Thread):
    ...
    def set_refresh_time(self, time):  # To be called to set how often the variables should be reset.
        if time > 10:
            self.refreshTime = time
        else:
            self.refreshTime = 10

    def set_visibility(self, status=True):
        self.mutex.lock()
        self.visible = status
...
def get_string_from_file(path):  ##This is the first function to be called for reading configurations for a xml file
    try:
...
def get_data_from_string(data, logger):  # Data is a string in xml format containing information
    components = []
    try:
    ...
    if root is not None:
        components, network_settings = parse_rcmanager(root, logger)
    xmldoc.freeDoc()
    return components, network_settings


def parse_rcmanager(node, logger):  # Call seperate functions for general settings and nodes
    components = []
    general_settings = NetworkValues()
        ...
        while child is not None:
            if child.type == "element":
                if child.name == "generalInformation":
                    parse_general_information(child, general_settings, logger)
                elif child.name == "node":
                    # print "parseing node"
                    parse_node(child, components, general_settings, logger)
                elif string_is_useful(str(node.properties)):
                    print 'ERROR when parsing rcmanager: ' + str(child.name) + ': ' + str(child.properties)
            child = child.next
    return components, general_settings


def parse_general_information(node, general_settings,
                              logger):  ##Takes care of reading the general information about the network tree
            ...
            if child.type == "element":
                if child.name == "group":
                    ...
                    try:
                        ...
                        group.setName(parse_single_value(child, "name"))
                        group.set_icon_file_path(parse_single_value(child, "iconfile"))
                        group.read_from_icon_file()
                        general_settings.Groups.append(group)
                    except Exception, e:
                        logger.logData("Couldn Read group " + str(e), "R")
                    else:
                        logger.logData("Read the group ::" + group.group_name + " Information")
                elif child.name == "simulation":
                    general_settings.spring_length = float(parse_single_value(child, "springlength"))
                    general_settings.field_force_multiplier = float(parse_single_value(child, "fieldforce"))
                    general_settings.hookes_constant = float(parse_single_value(child, "hookes"))
                    general_settings.roza = 1 - float(parse_single_value(child, "friction"))
...
def check_for_more_properties(node):
    if node.properties is not None: print 'WARNING: Attributes unexpected: ' + str(node.properties)


def search_for_group_name(settings, name):
    flag = 0
...
def parse_node(node, components, generalSettings, logger):  # To get the properties of a component
    if node.type == "element" and node.name == "node":
        child = node.children
        comp = CompInfo()
        # comp.CheckItem.setLogger(logger)
        comp.alias = parse_single_value(node, 'alias', False)
        # print "Started reading component:: "+comp.alias
        comp.DirectoryItem.setText(comp.alias)
        comp.endpoint = parse_single_value(node, 'endpoint', False)
        while child is not None:
            if child.type == "element":
                if child.name == "group":
                    comp.groupName = parse_single_value(child, "name")
                    try:
                        group = search_for_group_name(generalSettings, comp.groupName)
                    ...
                    comp.workingdir = parse_single_value(child, 'path')
                elif child.name == "upCommand":
                    comp.compup = parse_single_value(child, 'command')
                elif child.name == "downCommand":
                    comp.compdown = parse_single_value(child, 'command')
                elif child.name == "configFile":
                    comp.configFile = parse_single_value(child, 'path')
                elif child.name == "radius":  ##To be backward compatible with older tool
                    pass
                elif child.name == "xpos":
                    x = parse_single_value(child, 'value')
                    try:
                        comp.x = float(x)
                    except:
                        logger.logData("Error in Reading Position Value of " + comp.alias, "R")
                elif child.name == "ypos":
                    y = float(parse_single_value(child, 'value'))
                    try:
                        comp.y = float(y)
                    except:
                        logger.logData("Error in Reading Position Value of " + comp.alias, "R")
                elif child.name == "color":
                    comp.nodeColor = get_color_from_string(parse_single_value(child, "value"))
                elif child.name == "dependence":
                    comp.dependences.append(parse_single_value(child, 'alias'))
                elif child.name == "ip":
                    comp.Ip = parse_single_value(child, "value")
                elif string_is_useful(str(child.properties)):
                    print 'ERROR when parsing rcmanager: ' + str(child.name) + ': ' + str(child.properties)
            child = child.next

        check_for_comp_children(comp, node)  ##This function si there to verify All neccessary details are added
        components.append(comp)

    elif node.type == "text":
        if string_is_useful(str(node.properties)):
...
def parse_single_value(node, arg, doCheck=True, optional=False):
    if node.children is not None and doCheck is True:
        print 'WARNING: No children expected' + str(node)
...
def get_xml_from_network(network_settings, components, logger):
    string = ''
    string = string + '<?xml version="1.0" encoding="UTF-8"?>\n'
    string = string + '<rcmanager>\n\n'
    string = string + '\t<generalInformation>\n'

    for x in network_settings.Groups:
        string = string + '\t\t<group name="' + x.groupName + '" iconfile="' + x.groupIconFilePath + '" />\n'

    string = string + '\t\t<simulation hookes="' + str(network_settings.hookes_constant) + '" friction="' + str(
        1 - network_settings.roza) + '" springlength="' + str(network_settings.spring_length) + '" fieldforce="' + str(
        network_settings.field_force_multiplier) + '"/>\n'
    string = string + '\n\t</generalInformation>\n'
...
def write_to_file(the_file, string):  # Write a line to the file
    the_file.write((string).encode("utf-8"))
...
def check_for_comp_children(comp,
                            Xmlnode):  # This will check whether the opened Doc have enough children to draw the things
    return


def search_for_child(xmlnode, name):  ##Will search in tree for node with name Name
...
def up_component(component, logger):  # Just Up the component
    try:
        if component.CheckItem.haveStarted() is False:
            component.CheckItem.initializeComponent()
        if component.CheckItem.haveStarted() is False:
            logger.logData(
                "Component " + component.alias + " Cannot be Monitored because of bad Proxy setting(Error ignored)",
                "R")
...
def down_component(component, logger):  # To down a particular component
    try:
        proc = QtCore.QProcess()
