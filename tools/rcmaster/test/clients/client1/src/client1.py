...
if __name__ == '__main__':
	...
	proxyData["test"] = {"comp":"client31","caster":testPrx.checkedCast,"name":"test"}
	try:
		while True:
			try:
	...
	proxyData["test2"] = {"comp":"client32","caster":testPrx.checkedCast,"name":"test"}
	try:
		while True:
			try:
				interfaces = rcmaster_proxy.getComp(proxyData["test2"]["comp"],"localhost");
				iface = [x for x in interfaces if x.name == proxyData["test2"]["name"]][0]
				basePrx = ic.stringToProxy(proxyData["test2"]["name"]+":"+iface.protocol+" -h localhost -p "+str(iface.port))
				proxyData["test2"]["proxy"] = proxyData["test2"]["caster"](basePrx)
			except ComponentNotFound:
				print 'waiting for test2 interface'
				time.sleep(1)
			except IndexError:
				raise Exception(proxyData["test2"]["comp"]+" dosnt provide"+proxyData["test2"]["name"])
				time.sleep(3)
			except Ice.Exception:
				print 'Cannot connect to the remote object (test)'
				traceback.print_exc()
