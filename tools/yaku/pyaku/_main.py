...
def get_last_child(pid):
    ...
    while True:
        ...
        if not child_pids:
            ...
            if len(child_pids.splitlines())>1:
                result = []
...
class KonsoleSession:
    def __init__(self, terminal_id):
        ...
        logger.debug(f"Created KonsoleSession with fpid:{self.foreground_pid} spid:{self.session_pid} child_pid:{self.child_pid}")


    @property
    def current_directory(self):
        logger.debug(f"Current directory for KonsoleSession {self.konsole_session_id}")
        if not isinstance(self.child_pid, int):
            ...
            logger.debug(f"$ {pwdx_command}")
            pwd, error = get_command_return(pwdx_command)
            if pwd:
                pwd = pwd.split()[1].strip()
            else:
                print(f"Error while getting current directory for terminal {self.terminal_id}: pwdx {error.decode('utf-8')}")
            self.__current_directory = pwd.decode("utf-8")
        logger.debug(f"'{self.__current_directory}'")
        return self.__current_directory

    @property
    def last_command(self):
        logger.debug(f"Getting last command for KonsoleSession {self.konsole_session_id}")
        ...
        logger.debug(f"'{self.__last_command}'")
        return self.__last_command

    @property
    def running_command(self):
        logger.debug(f"Running command for KonsoleSession {self.konsole_session_id}")
        if self.foreground_pid != self.session_pid:
    ...
    def get_last_executed_command(self, terminal_id):
        ...
        YakuakeDBus().sessions.runCommandInTerminal(
            int(terminal_id), f"  echo \"!-1\" > {tmp_path}"
        )
        sleep(0.1)
...
class YakuakeTabStack:
    ...
    def load_open_tabs_info(self):
        logger.debug("Trying to load info of open tabs")
        next_index = 0
        while (session_id := self.session_at_tab(next_index)) != -1:
            tab = YakuakeTab(next_index, session_id)
            self.tabs_by_index[next_index] = tab
            self.tabs_by_session[session_id] = tab
            next_index += 1
        logger.debug(f"Found {len(self.tabs_by_index)} tabs")
    ...
    def load_tab_info(self, tab):
        logger.debug(f"Getting info for tab {tab.yakuake_session_id}")
        tab.title = self.tab_title(tab.yakuake_session_id)
        logger.debug(f"Current title is {tab.title}")
        tab.terminal_ids = self.terminal_ids_for_index(tab.index)
        logger.debug(f"Terminal ids {tab.terminal_ids}")
