...
# \mainpage RoboComp::batteryControl
#
...
#
# Just: "${PATH_TO_BINARY}/batteryControl --Ice.Config=${PATH_TO_CONFIG_FILE}"
#
...
if __name__ == '__main__':
    ...
    # Create a proxy to publish a BatteryLevel topic
    topic = False
    try:
        topic = topicManager.retrieve("BatteryLevel")
    except:
        pass
    while not topic:
        try:
            ...
            try:
                topic = topicManager.create("BatteryLevel")
            except:
                print('Another client created the BatteryLevel topic? ...')
    pub = topic.getPublisher().ice_oneway()
    batterylevelTopic = RoboCompBatteryLevel.BatteryLevelPrx.uncheckedCast(pub)
    mprx["BatteryLevelPub"] = batterylevelTopic
