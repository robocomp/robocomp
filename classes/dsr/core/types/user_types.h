...
namespace DSR {
    ...
    private:
        ...
        Edge(uint128_t from, uint128_t to, const std::string& type, uint32_t agent_id, const  std::map<std::string, Attribute> &attrs)
    ...
    public:
        ...
        [[deprecated("Use Edge::create<example_edge_type>(...)")]] Edge(uint128_t mTo, uint128_t mFrom, std::string mType, uint32_t mAgentId) : m_to(mTo), m_from(mFrom), m_type(std::move(mType)), m_attrs{},
                                  m_agent_id(mAgentId)
        {
        ...
        [[deprecated("Use Edge::create<example_edge_type>(...)")]] Edge(uint128_t mTo, uint128_t mFrom, std::string mType,
        ...
        static Edge create(uint128_t to, uint128_t from, std::string type, uint32_t agent_id)
        {
        ...
        template <typename edge_type>
        static Edge create(uint128_t to, uint128_t from, std::string type, const  std::map<std::string, Attribute> &attrs, uint32_t agent_id)
        {
            static_assert(edge_type::edge_type, "Invalid Edge type.");
        ...
        void to(uint128_t  mTo);

        void from(uint128_t  mFrom);
    ...
    private:
        ...
        Node(const std::string& type, uint32_t agent_id,
             ...
             const  std::map<std::pair<uint128_t, std::string>, Edge > &fano, const std::string& name = "")
    ...
    public:
        ...
        [[deprecated("Use Node::create<example_node_type>(...)")]] Node(std::string mType, uint32_t mAgentId,
                   const  std::map<std::string, Attribute> &mAttrs,
                   const  std::map<std::pair<uint128_t, std::string>, Edge > &mFano)
        ...
        static Node create(uint32_t agent_id,
                           const  std::map<std::string, Attribute> &attrs,
                           const  std::map<std::pair<uint128_t, std::string>, Edge > &fano,
        ...
        [[nodiscard]] const std::map<std::pair<uint128_t, std::string>, Edge > &fano() const;

        [[nodiscard]] std::map<std::pair<uint128_t, std::string>, Edge > &fano();
        ...
        void fano(const std::map<std::pair<uint128_t, std::string>, Edge > &mFano);
    ...
    private:
        ...
        std::string m_name;
        std::map<std::string, Attribute> m_attrs;
        std::map<std::pair<uint128_t, std::string>, Edge > m_fano;
        uint32_t m_agent_id = 0;
    };
