...
public slots:
    ...
    {
        std::string reference = to_string;
        if (combo_text == "world")
            reference = graph->get_node_root().value().name();
        std::optional<Mat::Vector6d> transform = inner_eigen->transform_axis(reference, from_string);
